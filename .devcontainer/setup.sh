...
# plugins, and the XULE plugin (cloned from xbrlus/xule, since this
# checkout is the XULE Editor VS Code extension, not the Arelle plugin
# itself).
set -euo pipefail
...
echo "==> Installing the XULE plugin (xbrlus/xule)"
$SUDO rm -rf \
  "$SITE_PACKAGES/arelle/plugin/xule" \
  "$SITE_PACKAGES/arelle/plugin/semanticHash.py" \
  "$SITE_PACKAGES/arelle/plugin/validate/DQC.py" \
  /tmp/xule
git clone --depth=1 https://github.com/xbrlus/xule.git /tmp/xule
$SUDO mkdir -p "$SITE_PACKAGES/arelle/plugin/validate"
$SUDO cp -a /tmp/xule/plugin/xule "$SITE_PACKAGES/arelle/plugin/xule"
$SUDO cp -a /tmp/xule/plugin/semanticHash.py "$SITE_PACKAGES/arelle/plugin/semanticHash.py"
$SUDO cp -a /tmp/xule/plugin/validate/DQC.py "$SITE_PACKAGES/arelle/plugin/validate/DQC.py"
...
echo "==> Applying this checkout's XULE Editor .vscode settings to dqc_us_rules/source/us/$LATEST_YEAR"
if [ -d "$REPO_ROOT/source/us/$LATEST_YEAR/.vscode" ]; then
  rm -rf "$DQC_TARGET/source/us/$LATEST_YEAR/.vscode"
  cp -a "$REPO_ROOT/source/us/$LATEST_YEAR/.vscode" "$DQC_TARGET/source/us/$LATEST_YEAR/.vscode"
else
  echo "Warning: no .vscode folder found for year $LATEST_YEAR in $REPO_ROOT/source/us; skipping"
fi

echo "==> Writing XULE Editor settings.json at the repo root"
